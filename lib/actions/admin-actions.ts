...
export async function runMatchingAction() {
  ...
  const availabilityByUser: Record<string, { time_slot_id: number, slot_index: number }[]> = {}
  ...
  for (const group of groups) {
    const startTime = slotIndexToTime(group.window.startIndex, group.window.day)
    const endTime   = slotIndexToTime(group.window.startIndex + 2, group.window.day)
    ...
    if (memberError) {
      console.error('Error inserting members for group', newGroup.id, memberError)
    }
  ...
  return {
    ...
    flagged: flagged.map((s: any) => ({ user_id: s.user_id, full_name: s.full_name })),
  }
}